...
import java.util.*
/**
 * 仿小米时钟 https://www.jianshu.com/p/eb8664405454
 */
...
constructor(
    ...
    //半透明的白色
    private val color_halfWhite: Int = android.graphics.Color.argb(255 - 180, 255, 255, 255)
    //纯白色
    private val color_white: Int = android.graphics.Color.parseColor("#FFFFFF")
    // 宽高
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        Log.e("onMeasure","width:$width height:$height imageSize:$imageSize")
    }
    ...
    override fun onDraw(canvas: Canvas?) {
        ...
        setCameraRotate()
        drawArcCircle(mCanvas)
        drawOutText(mCanvas)
        drawCalibrationLine(mCanvas)
        drawSecond(mCanvas)
        drawMinute(mCanvas)
        drawHour(mCanvas)
        drawBall(mCanvas)
    ...
    private fun drawArcCircle(canvas: Canvas) {
        val min = Math.min(width.toInt(), mHeight.toInt())
        val rect = RectF(-(min - paddingOut) / 2, -(min - paddingOut) / 2, (min - paddingOut) / 2, (min - paddingOut) / 2)
        canvas.drawRect(RectF((-min / 2).toFloat(), (-min / 2).toFloat(), ((min) / 2).toFloat(), ((min) / 2).toFloat()),mPaintOutCircle)
    ...
    private fun drawCalibrationLine(canvas: Canvas) {
        val min = Math.min(width.toInt(), mHeight.toInt()) / 2
        for (i in 0 until 360 step 2) {
            canvas.save()
            canvas.rotate(i.toFloat())
            //canvas.drawLine(min.toFloat() * 3 / 4, 0f, min * 3 / 4 + dp2px(10f), 0f, mPaintProgressBg)
            canvas.drawLine(0f, min.toFloat() * 3 / 4, 0f, min * 3 / 4 + dp2px(10f), mPaintProgressBg);
            //canvas.drawLine(min.toFloat() * 3 / 4, 0f, min * 3 / 4 + dp2px(10f), 0f, mPaintProgressBg);
            canvas.restore()
        }
    }
    ...
    private fun drawSecond(canvas: Canvas) {
        ...
        for (i in 0..90 step 2) {
            ...
            canvas.save()
            mPaintProgress.setARGB((255 - 2.8 * i).toInt(), 255, 255, 255)
